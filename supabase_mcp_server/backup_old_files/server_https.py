#!/usr/bin/env python3
"""
HTTPS-based MCP Server for Custom Connector
Runs as an HTTPS service that Claude Desktop can connect to via URL
"""
...
def create_self_signed_cert():
    ...
    try:
        ...
        subject = issuer = x509.Name([
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Supabase MCP Server"),
...
def main():
    ...
    if not os.path.exists("server.crt") or not os.path.exists("server.key"):
        ...
        if not create_self_signed_cert():
            ...
            print(f"🚀 Supabase MCP Server started on https://localhost:{port}")
    else:
    ...
    print(f"🔗 MCP endpoint: https://localhost:{port}/mcp")
